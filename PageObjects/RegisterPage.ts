...
class RegisterPage {
  ...
  async selectDay(day) {
    ...
    await t.click(this.DateOfBirthDayList).click(DaysOption.withText(day));
  }

  async selectMonth(month) {
    const DaysOption = this.DateOfBirthMonthList.find("option");
    await t.click(this.DateOfBirthMonthList).click(DaysOption.withText(month));
  }

  async selectYear(year) {
    const DaysOption = this.DateOfBirthYearList.find("option");
    await t.click(this.DateOfBirthYearList).click(DaysOption.withText(year));
  }
}
